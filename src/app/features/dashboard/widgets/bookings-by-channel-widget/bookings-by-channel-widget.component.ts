...
export class BookingsByChannelWidgetComponent implements AfterViewInit, OnDestroy {
  ...
  private createChart(): void {
    ...
    // Ensure labels are outside with connector lines and fairly close to the pie
    this.series.set('alignLabels', true);
    this.series.set('radius', am5.percent(70)); // slightly smaller pie to make room for labels
    ...
    this.series.labels.template.setAll({
      ...
      fontWeight: '600',
      populateText: true,
      inside: false, // use outer labels so connector lines are drawn
      radius: 10, // keep labels close to the chart
    });

    // Make connector lines visible on dark background
    this.series.ticks.template.setAll({
      stroke: am5.color('rgba(255, 255, 255, 0.35)'),
      strokeOpacity: 1,
      length: 8,
    });
